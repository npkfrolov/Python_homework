...
conn = psycopg2.connect(f"dbname='base' user='npkfrolov' password='123308' host='130.193.54.149' port='5432'")
print("Database opened successfully")
cur = conn.cursor()
cur.execute('SELECT * FROM arabs.abu_meditterenian_calc WHERE p1_fid < p2_fid')
dbrecords_calc = [x for x in cur.fetchall() if x[1]=='Ашкелон' and x[2]=='Акко'] # cur.fetchall() #
...
for row in dbrecords_calc:  # по строкам БД, в строке одна пара городов
    ...
    for ind, el in enumerate(template):  # перебираем варианты длины большой полуоси для каждого значения малой,
        ...
        for num, x in enumerate(el):
            ...
            semi_minor_axis = int(axis_b_range[ind])
        #     print(ind, semi_major_axis)
            ...
            cur.execute('''SELECT ST_distanceSpheroid(%s, %s, 'SPHEROID["USER", %s, %s]')''', (point1, point2, semi_major_axis, el[num]))
            ...
            # if pairs == 'Ашкелон - Акко':
            #     if int(dbrecords_distances[0][0]):
            if semi_major_axis == 4500001 and semi_minor_axis == 6090000:
